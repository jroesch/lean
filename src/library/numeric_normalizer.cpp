...
namespace lean {
bool numeric_normalizer_context::is_numeral(expr const & e) const {
    return is_num(e);
}

bool numeric_normalizer_context::is_nat_const(expr const & e) const {
    return is_constant(e) && const_name(e) == get_nat_name();
}

bool numeric_normalizer_context::is_neg_app(expr const & e) const {
    return is_const_app(e, get_neg_name(), 3);
}

bool numeric_normalizer_context::is_div(expr const & e) const {
    return is_const_app(e, get_div_name(), 4);
}

expr numeric_normalizer_context::mk_const(name const & n) {
    return mk_constant(n, m_ainst.get_levels());
}

expr numeric_normalizer_context::mk_cong(expr const & op, expr const & type, expr const & a,
...
pair<expr, expr> numeric_normalizer_context::mk_norm_add(expr const & lhs, expr const & rhs) {
...
pair<expr, expr> numeric_normalizer_context::mk_norm_add1(expr const & e) {
    buffer<expr> args;
...
pair<expr, expr> numeric_normalizer_context::mk_norm_mul(expr const & lhs, expr const & rhs) {
    buffer<expr> args_lhs;
    buffer<expr> args_rhs;
    expr lhs_head = get_app_args (lhs, args_lhs);
    expr rhs_head = get_app_args (rhs, args_rhs);
...
optional<mpq> numeric_normalizer_context::to_mpq(expr const & e) {
    auto v = to_num(e);
    if (v) {
...
mpq numeric_normalizer_context::mpq_of_expr(expr const & e) {
...
pair<expr, expr> numeric_normalizer_context::get_type_and_arg_of_neg(expr const & e) {
...
expr numeric_normalizer_context::mk_norm_eq_neg_add_neg(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
...
expr numeric_normalizer_context::mk_norm_eq_neg_add_pos(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    lean_assert(is_neg_app(s_lhs));
    lean_assert(!is_neg_app(s_rhs));
...
expr numeric_normalizer_context::mk_norm_eq_pos_add_neg(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    lean_assert(is_neg_app(s_rhs));
    lean_assert(!is_neg_app(s_lhs));
...
expr numeric_normalizer_context::mk_norm_eq_pos_add_pos(expr const & s_lhs, expr const & s_rhs, expr const & DEBUG_CODE(rhs)) {
    lean_assert(!is_neg_app(s_lhs));
    lean_assert(!is_neg_app(s_rhs));
    lean_assert(!is_neg_app(rhs));
...
expr numeric_normalizer_context::mk_norm_eq_neg_mul_neg(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    lean_assert(is_neg_app(s_lhs));
    lean_assert(is_neg_app(s_rhs));
    lean_assert(!is_neg_app(rhs));
    auto s_lhs_v = get_type_and_arg_of_neg(s_lhs).second;
...
expr numeric_normalizer_context::mk_norm_eq_neg_mul_pos(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    lean_assert(is_neg_app(s_lhs));
    lean_assert(!is_neg_app(s_rhs));
    lean_assert(is_neg_app(rhs));
    expr s_lhs_v, type;
...
expr numeric_normalizer_context::mk_norm_eq_pos_mul_neg(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    lean_assert(!is_neg_app(s_lhs));
    lean_assert(is_neg_app(s_rhs));
    lean_assert(is_neg_app(rhs));
    expr s_rhs_v, type;
...
// returns a proof that s_lhs + s_rhs = rhs, where all are nonneg normalized numerals
expr numeric_normalizer_context::mk_norm_eq_pos_mul_pos(expr const & s_lhs, expr const & s_rhs, expr const & DEBUG_CODE(rhs)) {
    lean_assert(!is_neg_app(s_lhs));
    lean_assert(!is_neg_app(s_rhs));
    lean_assert(!is_neg_app(rhs));
...
expr numeric_normalizer_context::mk_norm_div_add(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
...
expr numeric_normalizer_context::mk_norm_add_div(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
...
expr numeric_normalizer_context::mk_nonzero_prf(expr const & e) {
    buffer<expr> args;
    expr f = get_app_args(e, args);
...
expr numeric_normalizer_context::mk_pos_prf(expr const & e) {
    buffer<expr> args;
    get_app_args(e, args);
    expr type = args[0];
    expr prf;
...
expr numeric_normalizer_context::mk_nonneg_prf(expr const & e) {
    buffer<expr> args;
    get_app_args(e, args);
    expr type = args[0];
    expr prf;
...
// s_lhs is div. returns proof that s_lhs * s_rhs = rhs
expr numeric_normalizer_context::mk_norm_div_mul(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    buffer<expr> args;
...
expr numeric_normalizer_context::mk_norm_mul_div(expr const & s_lhs, expr const & s_rhs, expr const & rhs) {
    buffer<expr> args;
...
expr_pair numeric_normalizer_context::mk_norm_nat_sub(expr const & s_lhs, expr const & s_rhs) {
...
pair<expr, expr> numeric_normalizer_context::mk_norm(expr const & e) {
    buffer<expr> args;
    expr f = get_app_args(e, args);
