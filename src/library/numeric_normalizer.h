...
namespace lean {
class numeric_normalizer_context {
    type_context & m_ctx;
...
public:
    numeric_normalizer_context(type_context & ctx): m_ctx(ctx), m_ainst(ctx) {}

    pair<expr, expr> mk_norm(expr const & e);
};

inline pair<expr, expr> numeric_normalize(type_context & ctx, expr const & e) {
    return numeric_normalizer_context(ctx).mk_norm(e);
}
}
